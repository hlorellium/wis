...
export class BackgroundRenderer {
    ...
    render(ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement, state: State) {
        ...
        const startX = Math.floor(-state.view.panX / state.view.zoom / this.gridGap) * this.gridGap;
        const startY = Math.floor(-state.view.panY / state.view.zoom / this.gridGap) * this.gridGap;
